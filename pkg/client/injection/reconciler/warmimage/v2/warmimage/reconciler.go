...
import (
	...
	warmimagev2 "github.com/mattmoor/warm-image/pkg/client/listers/warmimage/v2"
	zap "go.uber.org/zap"
...
// controller reconciling v2.WarmImage.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v2.WarmImage. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v2.WarmImage) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v2.WarmImage.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v2.WarmImage. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v2.WarmImage) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v2.WarmImage if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v2.WarmImage.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v2.WarmImage) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v2.WarmImage if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v2.WarmImage.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v2.WarmImage) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v2.WarmImage) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v2.WarmImage resources.
type reconcilerImpl struct {
	...
	Lister warmimagev2.WarmImageLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister warmimagev2.WarmImageLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.WarmImages(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v2.WarmImage, desired *v2.WarmImage) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.MattmoorV2().WarmImages(desired.Namespace)
		...
		updater := r.Client.MattmoorV2().WarmImages(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v2.WarmImage) (*v2.WarmImage, error) {

	getter := r.Lister.WarmImages(resource.Namespace)
	...
	patcher := r.Client.MattmoorV2().WarmImages(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v2.WarmImage) (*v2.WarmImage, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v2.WarmImage, reconcileEvent reconciler.Event) (*v2.WarmImage, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
